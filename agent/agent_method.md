...
*这一份既是可以看的说明，也是可以照着讲的稿子。*
*正文英文是**讲的时候直接念的**；`中：` 开头的是**中文对照**，帮你确认讲到哪、以及为什么这么做。*
*所有图都在同一个文件夹里，md 阅读器会直接显示。*
...
| 步骤 | 内容 |
|---|---|
| [Step 0](#step-0--the-input) | 输入数据 |
| [Overview](#overview--the-eight-steps) | 八步总览 |
| [Step 1](#step-1--identify-the-trucks-and-the-zones) | 识别卡车和区域 |
| [Step 2](#step-2--build-cycles) | 建周期 |
| [Step 3](#step-3--cut-each-cycle-into-four-phases) | 切成四段 |
| [Step 3b](#step-3b--a-phase-duration-is-not-a-driving-duration) | 段时长 ≠ 行驶时长 |
| [Step 4](#step-4--find-the-places-on-the-route) | 找出线路上的地点 |
| [Step 5](#step-5--assign-every-minute-to-exactly-one-place) | 每一分钟归到唯一一个地点 |
| [Step 6](#step-6--delay--actual--reference) | **延误 = 实际 − 参照** |
| [Step 7](#step-7--how-fast-each-station-can-go) | 站点速率和利用率 |
| [Step 8](#step-8--convert-delay-hours-into-loads-per-day) | 延误小时 → 每天几车 |
| [Output](#output--result-for-bn-november-2025) | 结果、四条规则、最终建议 |
| [Checks](#checks--five-months) | 五个月、选定值、局限 |
...
`中：` 四个文件，全部来自矿方已有的跟踪系统。**没有载重、油耗、调度记录**，所以单位只能是趟数和卡车小时。**104 个区域只有 11 个有真多边形**，其余用"框 + 100 米缓冲"近似——这条限制后面会反复出现。
...
> **Say:** Eight steps. Identify trucks and zones. Build cycles. Cut each cycle into four phases. Find out which places lie on the route and classify them. Assign every minute to exactly one place. Compute delay as actual minus a reference, per phase. Measure how fast each station can go and how heavily it is used. Then convert the delays into loads per day and rank them.

`中：` 八步，每一步吃上一步的输出。**没有任何一步需要人告诉它某个地点是干什么用的**——这一点在第 4 步会具体看到。
...
> **Say:** Four phases, defined purely by the four timestamps. Haul loaded, from leaving the load zone to arriving at the dump. Dump. Return empty, from leaving the dump to arriving back. And dwell at the load zone, which covers loading itself and any waiting to be loaded. BN means are 75, 19, 88 and 50 minutes.
...
`中：` 这一步是"agent 自己搞清楚这条线路长什么样"。**两个判据都不读地名**，所以换一个矿理论上也能用。图上站点和路过点的停车率之间是空的（0.14 到 0.91 之间没有任何地点），所以 0.5 这条分界线取多少都一样。
...
> **Say:** This is the core of the method, so I'll go through the table row by row. Every phase has a measured quantity and a reference, and the delay is the difference.
>
...
`中：` 这一节是你最该讲透的。要点两句：**参照全部来自车队自己**（不是外部标准），**站点的参照是量出来的不是挑出来的**（没有别的车在场时的中位逗留）。
...
> **Say:** The dwell at the load zone mixes four different things, and the test that separates them is not how long the truck waited. It is how many other trucks were already inside when it arrived.
>
...
> **Say:** To know how fast a station can go, sort its departures and take the gap between consecutive ones. Keep only gaps under thirty minutes — a longer gap is a period when the station was idle waiting for a truck, so including it would measure arrivals rather than capacity.
>
...
> **Say:** The delays are in truck-hours; to rank them we need loads per day. Start from an identity: cycling truck-hours per day equals loads per day times loop time over sixty — 312 equals 80.6 times 232.2 over sixty. Read the other way, loads per day equals truck-hours times sixty over loop time.
>
...
> **Say:** Here is what comes out. The shovel is at 50 percent utilisation and 1 374 truck-hours of queue sit in front of it. Queueing in front of a station that is idle half the time points to uneven arrivals rather than insufficient loading capacity.
>
...
> **Say:** Same code on five months, nothing adjusted between them. Loads per day range from 48 to 88. Shovel utilisation from 0.43 to 0.59 — never near full. The share of phase time actually driving stays between 56 and 58 percent. The empty-return road is seventh or eighth every month. And first place is named in two of the five months and reported as a tie in three.
