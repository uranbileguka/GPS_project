...
from gps_lib import config                                  # resolves GPS_DATA_DIR (.env) same as every notebook
CYCLES_CSV = os.path.join(config.DATA_DIR, 'cycles_all_months.csv')
...
ZONE_ALIASES = {'bn': 25559, 'middling': 25385, 'reject': 25384}   # friendly name -> zone_id


def resolve_zone(zone) -> int:
    """Accept either a zone_id (int or numeric string) or a friendly name ('bn'/'middling'/'reject')."""
    if isinstance(zone, str) and not zone.isdigit():
        try:
            return ZONE_ALIASES[zone.strip().lower()]
        except KeyError:
            raise ValueError(f"Unknown flow {zone!r} — use one of {list(ZONE_ALIASES)} or a zone_id: {list(ZONES)}")
    return int(zone)
...
# ================================================================ L1 perception
def perception(zone_id, month=None, start=None, end=None):
    """month='2025-11' picks a whole calendar month; start=/end= ("YYYY-MM-DD")
    picks any date range instead (end is inclusive of that whole day). Exactly
    one of month or start/end should be given."""
    C = pd.read_csv(CYCLES_CSV, parse_dates=['depart_load','arrive_unload',
                                             'depart_unload','arrive_load'])
    z = ZONES[zone_id]
    mask = (C['region']==z['region']) & (C['load_zone']==zone_id)
    if start is not None or end is not None:
        if start is not None:
            mask &= C['depart_load'] >= pd.Timestamp(start)
        if end is not None:
            mask &= C['depart_load'] < pd.Timestamp(end) + pd.Timedelta(days=1)
    elif month is not None:
        mask &= C['month'] == month
    else:
        raise ValueError("perception() needs either month= or start=/end=")
    cyc = C[mask].copy()
    if cyc.empty:
        raise ValueError(f"No cycles for zone {zone_id} in that period "
                         f"(month={month!r}, start={start!r}, end={end!r}) — check the range against the data (Jul-Nov 2025).")
...
# ================================================================ orchestrator
def diagnose(zone_id, month=None, start=None, end=None):
    """month='2025-11' picks a whole calendar month (the default if nothing is
    given); start=/end= ("YYYY-MM-DD") picks any date range within the data
    (Jul-Nov 2025) instead — e.g. diagnose(25559, start="2025-11-01", end="2025-11-15")."""
    if month is None and start is None and end is None:
        month = '2025-11'
    period_label = month if (start is None and end is None) else f"{start or '...'} to {end or '...'}"
    z = ZONES[zone_id]
    cyc, unload_zone = perception(zone_id, month=month, start=start, end=end)
    ...
    return dict(
        zone_id=zone_id, name=z['name'], region=z['region'], month=period_label,
...
if __name__ == '__main__':
    import argparse
    ap = argparse.ArgumentParser(description="Layers 1-3: structured diagnosis + printed report.")
    ...
    zid = resolve_zone(a.zone)
    dx = diagnose(zid, month=a.month, start=a.start, end=a.end)
    print(report(dx))
